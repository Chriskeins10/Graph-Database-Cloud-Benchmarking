"""
workloads/falkordb/falkordb_lookups.py
Point Lookup + Filtered Lookup on FalkorDB
"""
...
def get_random_node_ids(g, count=300):
    result = g.query(
...
def run_query(g, query, params=None):
    start = time.perf_counter()
    g.query(query, params or {})
...
def main():
    ...
    queries = {
        ...
        "Filtered Lookup": {
            "query": "MATCH (n:User) WHERE toString(n.id) STARTS WITH $prefix RETURN count(n)",
    ...
    for name, item in queries.items():
        ...
        for _ in range(WARMUP):
            run_query(g, item["query"], item["params"]())

        for _ in range(ITERATIONS):
            latency = run_query(g, item["query"], item["params"]())
